 ...
 * <br><br>
 * 2) Durability. Writes get persisted to disk, so that this store maintains state even if the application closes or is killed.
 * <br><br>
 ...
 * <br><br>
 * Note that since writes are asynchronous, an in-flight write may be lost if the application is killed before the data has
 * been written to disk. If you require true 'commit' semantics then remember_lib.Remember is not for you.
 * <br><br>
    ...
    public static synchronized Remember init(Context context, String sharedPrefsName) {
        ...
        if (context == null || TextUtils.isEmpty(sharedPrefsName)) {
            throw new RuntimeException(
                    "You must provide a valid context and shared prefs name when initializing remember_lib.Remember");
        }
